...
app.post("/edit-blog/", upload.single("image"), editBlog);
...
async function home(req, res) {
  const query = `SELECT public.projects.id, public.projects.name, public.projects.start_date, public.projects.end_date, public.projects.description, public.projects.technologies, public.projects.image, public.projects.id, public.projects."createdAt", public.users.name AS user_name FROM public.users JOIN public.projects ON public.users.id=public.projects.user_id;`;
...
async function addBlog(req, res) {
  ...
  const query = `INSERT INTO projects(name, start_date, end_date, description, technologies, image, "createdAt", "updatedAt",user_id) VALUES('${name}', '${start_date}', '${end_date}', '${description}', '${technologiesString}', '${image}', now(), now(),${userId})`;
...
async function editBlog(req, res) {
  ...
  const query = `UPDATE projects SET name = '${name}', start_date = '${start_date}', end_date = '${end_date}', description = '${description}', technologies = '${technologiesString}',image = ${newImage ? `'${newImage}'` : 'image'},"createdAt" = now(), "updatedAt" = now() WHERE id = ${id}`;
